...
import { fetchPlayerComparison, fetchPlayerYearlyComparison } from '@/lib/actions/actions';
import { fetchAllPlayers } from '@/lib/actions/players/player';
import { useYear } from '@/contexts/year-context';
import type { PlayerWithAge, PlayerYearlyComparison } from '@/lib/db/db';
...
export default function CompareContent() {
	...
	return (
		<div className="flex flex-col gap-6">
			<div>
				<h3 className="text-xl font-bold text-foreground sm:text-2xl">Comparación de jugadores</h3>
								...
								{selectedIds.map((id, i) => {
									...
									return (
										...
										>
											...
											>
												<X className="h-3 w-3" />
												<span className="sr-only">Quitar {p.name}</span>
											</button>
										</div>
									);
								})}
            ...
            <TabsTrigger value="yearly" disabled>Year-by-Year</TabsTrigger> */}
					...
					<TabsContent value="yearly" className="flex flex-col gap-6">
						<YearlySection comparison={comparison} yearlyComparison={yearlyComparison} />
					</TabsContent>
				</Tabs>
			)}
		</div>
				...
				{comparison.map((c, i) => (
					<Card key={c.id}>
						<CardHeader className="pb-2">
							<CardTitle className="text-base" style={{ color: CHART_COLORS[i] }}>
								{c.name}
							</CardTitle>
							<CardDescription>Edad: {c.edad} años</CardDescription>
						</CardHeader>
						<CardContent>
...
function ChartsSection({ comparison }: { comparison: ComparisonData[] }) {
	...
	const radarData = [
		...
		},
		{
			stat: 'Goals/Match',
			...Object.fromEntries(
				comparison.map((c) => [c.name, +((c.goals_per_match / maxValues.gpm) * 100).toFixed(1)])
			),
		},
		{
			stat: 'Assists/Match',
			...Object.fromEntries(
				comparison.map((c) => [c.name, +((c.assists_per_match / maxValues.apm) * 100).toFixed(1)])
			),
		},
	];
		...
		<>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Side-by-side comparison of key statistics</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>Share of total playing time among compared players</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="h-[340px]">
							<ResponsiveContainer width="100%" height="100%">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Per-match output: goals and assists per game played</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Yellow and red card count per player</CardDescription>
				</CardHeader>
				<CardContent>
		...
		<>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>How each player{"'"}s goal tally evolved over the years</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				<Card>
					<CardHeader>
						<CardTitle className="text-base">Appearances - Year by Year</CardTitle>
					</CardHeader>
					<CardContent>
						<div className="h-[280px]">
							<ResponsiveContainer width="100%" height="100%">
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-base">Goals per Match - Year by Year</CardTitle>
					<CardDescription>Scoring efficiency trend over the years</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="h-[300px]">
						<ResponsiveContainer width="100%" height="100%">
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-base">Goal Contributions per Year</CardTitle>
					<CardDescription>Goals and assists stacked per player for each year</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="h-[340px]">
						<ResponsiveContainer width="100%" height="100%">
