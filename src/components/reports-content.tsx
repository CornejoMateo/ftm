...
import { fetchPlayersWithStats, fetchMonthlyStats, fetchTeamStats } from '@/lib/actions/actions';
...
export default function ReportsContent() {
	...
	const playerChartData = players.slice(0, 10).map((p) => ({
		name: `${p.name} ${p.last_name.charAt(0)}.`,
		goals: p.total_goals,
			...
			<div className="flex flex-col gap-4">
				<div>
					...
					<p className="text-sm text-muted-foreground">
						Detailed team and player performance reports{' '}
						{selectedYear ? `for ${selectedYear}` : 'for all time'}
					</p>
				</div>
			</div>
											...
											data={[
												{
													stat: 'Goals',
													...Object.fromEntries(
														top5.map((p) => [p.id, (p.total_goals / maxGoals) * 100])
													),
												},
												{
													stat: 'Assists',
													...Object.fromEntries(
														top5.map((p) => [p.id, (p.total_assists / maxAssists) * 100])
													),
												},
												{
													stat: 'Matches',
													...Object.fromEntries(
														top5.map((p) => [p.id, (p.matches_played / maxMatches) * 100])
													),
												},
												{
													stat: 'Minutes',
													...Object.fromEntries(
														top5.map((p) => [p.id, (p.total_minutes / maxMinutes) * 100])
													),
												},
											]}
										>
									...
									.map((p) => (
										<div key={p.id} className="rounded-lg border border-border p-3">
											<div className="flex items-center justify-between">
												...
												<p className="text-xs text-muted-foreground">{p.matches_played} matches</p>
											</div>
